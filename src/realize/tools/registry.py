...
TOOL_REGISTRY = {
    ...
    "update_campaign_lookalike_audience": {
        "description": (
            ...
            "\n"
            "Each item is {rule_id, similarity_level}. rule_id is the integer rule ID for a lookalike audience (from the Realize UI, Audiences > Lookalike). similarity_level (%) depends on the audience subtype — CRM accepts 5/10/15/20/25; pixel accepts 5; PBP (predictive) accepts 1/2/3/4/5. The server resolves the subtype from rule_id and rejects mismatches.\n"
            "\n"
            "PBP lookalike audiences cannot be created via this MCP server (no audience-management tool exposed); create them in the Realize UI. Once they exist, targeting them via this tool works the same as CRM and pixel lookalikes.\n"
            "\n"
